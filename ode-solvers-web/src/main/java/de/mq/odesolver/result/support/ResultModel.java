...
public class ResultModel {
	...
	private final Collection<? extends Result> results; 
	...
	void calculateInitialValues(final double[] kVector) {
		...
		IntStream.range(0, kVector.length).forEach(i -> initialValues.add(new SimpleImmutableEntry<>(String.format("k[%d]",i), kVector[i]  )));
	}


	private void addRangeDerivative(final Collection<Entry<String,double[]>> ranges, final Collection<? extends Result> results,final int yDerivative) {
		final Optional<Double> min = results.stream().map(r -> r.yDerivative(yDerivative)).min((x1,x2) -> x1.compareTo(x2));
		final Optional<Double> max = results.stream().map(r -> r.yDerivative(yDerivative)).max((x1,x2) -> x1.compareTo(x2));
		final StringBuffer text = new StringBuffer("y");
		IntStream.rangeClosed(1, yDerivative).forEach(i -> text.append("'"));
		if(min.isPresent()&&max.isPresent()){
			ranges.add(new SimpleImmutableEntry<>(text.toString(), FunctionResultImpl.doubleArray(min.get(),max.get())));
		}
	}
