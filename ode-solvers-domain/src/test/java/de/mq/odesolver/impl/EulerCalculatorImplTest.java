...
class EulerCalculatorImplTest {
	...
	private final OdeResultCalculator odeResultCalculator = new EulerCalculatorImpl(last -> last.yDerivative(0) - last.x());
	@Test
	...
	final void calculateforFirstOrderOdeString() {
		// Papuala Seite 234
		final OdeResultCalculator odeResultCalculator = new EulerCalculatorImpl("y[0]-x");
		final double[] y1 = odeResultCalculator.calculate(new OdeResultImpl(new double[] { 0 }, 0), 0.05);
		assertEquals(1, y1.length);
		assertEquals(0, y1[0]);
	...
	final void calculateforSecondOrderOdeString() {
		...
		final OdeResultCalculator odeResultCalculator = new EulerCalculatorImpl("-2*y[1]+3*y[0]");
